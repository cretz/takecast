...
func LoadKeyPairFromFiles(certFile string, keyFile string) (*KeyPair, error) {
	...
	return LoadKeyPairFromBytes(certPEM.Bytes, keyPEM.Bytes)
}

func LoadKeyPairFromBytes(certDERBytes []byte, keyPKCS1Bytes []byte) (*KeyPair, error) {
	privKey, err := x509.ParsePKCS1PrivateKey(keyPKCS1Bytes)
	if err != nil {
		return nil, fmt.Errorf("failed parsing PK: %w", err)
	}
...
func NewDefaultCertificateTemplate(subject pkix.Name) *x509.Certificate {
	...
	if err != nil {
		panic(fmt.Errorf("failed to create serial number: %w", err))
	}
...
func GenerateIntermediateCAKeyPair(
	...
	if err != nil {
		return nil, fmt.Errorf("failed parsing CA cert: %w", err)
	}
...
// template is mutated
func GenerateStandardKeyPair(parentCA *KeyPair, template *x509.Certificate, privKey *rsa.PrivateKey) (kp *KeyPair, err error) {
	kp = &KeyPair{PrivKey: privKey}
	if template == nil {
